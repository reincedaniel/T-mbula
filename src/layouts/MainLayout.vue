<template>
  <q-layout view="hHh LpR fFf">
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above :width="$q.platform.is.desktop ? 300 : $q.screen.width">
      <q-scroll-area style="height: calc(100% - 150px); margin-top: 150px; border-right: 1px solid #ddd">
        ...
        <q-list class="poppins-medium text-grey-9" padding>
          ...
          <q-separator />
          <q-item @click="rightDrawerOpen = false" to="/contacts" class="text-grey-8 text-bold" clickable v-ripple>
            <q-item-section avatar>
    ...
    <q-drawer v-if="mapStore.poi.length > 0" show-if-above v-model="rightDrawerOpen" side="right" bordered>
      <!-- drawer content -->
      <pre v-if="false">{{ mapStore.poi }}</pre>
          ...
          <template v-for="(menuItem, index) in mapStore.poi" :key="index">
            <q-item clickable v-ripple @click="getThePOI(menuItem)">
              ...
              <q-item-section>
                ...
                <q-item-label caption>Ratings Total: {{ humanStorageSize(menuItem.user_ratings_total) }}</q-item-label>
              ...
              <q-item-section side top>
                <q-badge :color="getTheRateColor(menuItem.rating)" :label="menuItem.rating" />
              </q-item-section>
            </q-item>
      ...
      <q-card class="q-pa-sm poppins-regular" style="width: 450px">
        ...
        <q-card-section class="row  col flex flex-center q-gutter-md">
          <q-btn @click="singInWithGoogle()" round outline no-caps icon="eva-google-outline" color="green-6"
            v-close-popup />
          <q-btn v-if="false" round outline no-caps icon="eva-facebook-outline" color="blue" v-close-popup />
